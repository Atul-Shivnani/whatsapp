...
const appendTranslateIconToFooter = () => {
    try {
        ...
        if (targetDiv && !targetDiv.querySelector('.translate-icon')) { // Only add if it doesn't exist already
            ...
            // Add event listener for the translate icon if any functionality is required on click
            translateIcon.addEventListener('click', async () => {
                ...
                if (messageContainer) {
                    ...
                    if (messageSpan) {
                        const messageText = messageSpan.innerText; // Changed from textContent to innerText
                        ...
                        if (messageText) {
                            ...
                            if (translatedText) {
                                ...
                                messageSpan.innerText = translatedText; // Update text here
...
const translateMessage = async (messageText, messageNode, preferredLanguage) => {
    try {
        const res = await fetch("http://localhost:5000/translate", {
            method: "POST",
            body: JSON.stringify({
                q: messageText, // Message to be translated
                source: "auto", // Automatically detect the source language
                target: preferredLanguage,   // Use the language from Chrome storage
                format: "text",
                alternatives: 3,
        ...
        // Add the translated text below the original message in the DOM
    ...
    mutations.forEach(mutation => {
        if (mutation.type === 'childList') {
            ...
            newMessages.forEach(messageNode => {
                if (messageNode.nodeType === 1) { // Ensure it's an Element Node
                    // Find the message text inside the new message node
                    ...
                    if (messageTextSpan) {
                        ...
                        console.log('New message:', messageText); // Log new messages

                        // Send the message for translation along with the node to append the translation
...
const fetchAllMessages = (preferredLanguage) => {
    const messages = document.querySelectorAll('div.message-in, div.message-out'); // Adjust the selector as needed
    messages.forEach(messageNode => {
        const messageTextSpan = messageNode.querySelector('span._ao3e.selectable-text.copyable-text');
        if (messageTextSpan) {
            const messageText = messageTextSpan.innerText;
            console.log('Existing message:', messageText); // Log existing messages

            // Send the existing messages for translation along with the node to append the translation
            translateMessage(messageText, messageNode, preferredLanguage);
        }
    });
};
...
const initializeChatObserver = (preferredLanguage) => {
    ...
    if (chatIntervalId) clearInterval(chatIntervalId); // Clear existing intervals before starting a new one

    // Check if the chat container is not found and retry every second
    chatIntervalId = setInterval(() => {
        chatContainer = getChatContainer();
        if (chatContainer) {
            clearInterval(chatIntervalId); // Stop the interval once the chat container is found
            console.log('Chat container found:', chatContainer);

            // Fetch all existing messages initially
            fetchAllMessages(preferredLanguage);

            // Fetch the footer and add the translate icon
            ...
            chatObserver = new MutationObserver((mutations) => handleNewMessages(mutations, preferredLanguage));
            chatObserver.observe(chatContainer, { childList: true, subtree: true }); // Observe child nodes and their subtree
    ...
    if (listItems.length > 0) {
        listItems.forEach(listItem => {
            listItem.addEventListener('click', () => {
                console.log('List item clicked:', listItem); // Log the clicked list item
                
                initializeChatObserver(preferredLanguage); // Call the function when a list item is clicked
            });
        });
...
const listItemIntervalId = setInterval(() => {
    chrome.storage.sync.get(['preferredLanguage'], (result) => {
        ...
        const listItemsAdded = setupListItemListeners(preferredLanguage);
        if (listItemsAdded) {
